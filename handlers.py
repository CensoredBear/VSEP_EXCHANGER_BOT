...
Если случился такой удивительный случай:пришла оплата по заявке из прошлой смены - обратитесь к оператору Сервиса (можете даже командой /sos) для попытки реанимировать заявку (предварительно найдите номер заявки в бланке расчёта с прошлой смены)
    ...
    task = asyncio.create_task(expire_control_buttons(message.bot, message.chat.id, message.message_id + 1, 120, base_text=base_text))  # +1 потому что reply
...
async def expire_control_buttons(bot: Bot, chat_id: int, message_id: int, delay_seconds: int, base_text: str = ""):
    ...
    try:
        ...
        new_text = base_text + (
    "\n\n** ℹ️ КОМАНДА КОНТРОЛЬ НЕ ЗАВЕРШЕНА МЕНЕДЖЕРОМ ℹ️ СРОК ДЕЙСТВИЯ КНОПОК ИСТЁК **"
    "\n\n🔴🔴🔴 ВНИМАНИЕ МЕНЕДЖЕРУ ПАРТНЁРА: НЕОБХОДИМО КОРРЕКТНО ВЫПОЛНИТЬ КОМАНДУ КОНТРОЛЯ!"
    "\nИначе заявка НЕ попадёт на проверку и НЕ будет включена в лист оплаты!"
)
